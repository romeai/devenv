...
- Fixed processes that exit on their own never reaching the Completed state under process-compose: the per-process `devenv-tasks` wrapper kept running after the wrapped process exited, so process-compose showed the process as still running forever. The wrapper, and `devenv up` with the native process manager, now exit once every process has finished ([#2879](https://github.com/cachix/devenv/issues/2879)).
...
- Replaced `tasks.<name>.execIfModified` with explicit task cache contracts. Cached tasks now validate required inputs and outputs, include task definitions, selected environment values, and dependency outputs in their identity, reject inputs that change during execution, preserve symlink semantics, and serialize concurrent runs of the same task across devenv processes.
- Lifecycle tasks now fail shell entry and `devenv up` when required initialization fails. Managed-file reconciliation and git-hook installation use task cache contracts, and installed prek hooks use worktree-relative configuration paths.
...
- **Task caching**: `tasks.<name>.execIfModified` was removed. Declare `tasks.<name>.cache.inputs`, `cache.outputs`, and `cache.env` explicitly instead.
